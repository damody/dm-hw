...
namespace OMP
{
	...
	void Model::Render_solid()
	{
		...
		for (f_it = Mesh.faces_begin(); f_it != Mesh.faces_end(); ++f_it) 
		{
			...
			{						
				glNormal3dv(Mesh.normal(fv_it.handle()));
				glVertex3dv(Mesh.point(fv_it.handle()));
			}
		}
		glEnd();		
		glDisable(GL_POLYGON_OFFSET_FILL);
	...
	void Model::Render_wireframe()
	{
		...
		for(e_it=Mesh.edges_begin(); e_it != Mesh.edges_end(); ++e_it)
		{
			...
			glVertex3dv(Mesh.point(Mesh.from_vertex_handle(_hedge)));
			glVertex3dv(Mesh.point(Mesh.to_vertex_handle(_hedge)));			
		}
		glEnd();
		glEnable(GL_LIGHTING);
	}
...
void Tri_Mesh::Render_Solid()
{
		...
		for (f_it = faces_begin(); f_it != faces_end(); ++f_it) 
		{
			...
			{						
				...
				glVertex3dv(point(fv_it.handle()));
			}
		}
		glEnd();	
